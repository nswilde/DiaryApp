...
/// A Class that updates, stores and syncs most of the data in this app,
/// including CoreData, iCloud, handling the creation, modification and
/// deletion of diary entries, tags and award data.
class DataController: ObservableObject {
    /// The lone CloudKit container used to store all our data.
    let container: NSPersistentCloudKitContainer

    // Spotlight Part 1/3: Create a variable to handle Spotlight feature,
    // allowing us to search within the app while we are on the homepage of iOS.
    // Part 2 @ DataController initializer
    ...
    init(inMemory: Bool = false, defaults: UserDefaults = .standard) {
        ...
        container.loadPersistentStores { [weak self] _, error in
            if let error {
                fatalError("Fatal access loading store: \(error.localizedDescription)")
            }

            // Spotlight Part 2/3: Initializes Spotlight function innately with our
            // DataController, description is used to allow Strings within iOS
            // to search our DataController with Spotlight, coordinator tells
            // the spotlight function how to discern where the data is located
            // in our app based on the source of the String input.
            // Part 3 @ DiaryApp.swift loadSpotlightItem func
    ...
    /// Queues an autosave request after a 3 second delay to ensure
    /// data is safe should an unexpected crash ever happen.
    func queueSave() {
        saveTask?.cancel()
    ...
    func newTag() -> Bool {
        // Should create checks for premium purchase and then decides whether
        // user should be allowed to make custom tags, or use the default free
        // ones.
        ...
        tag.id = UUID()
        tag.name = NSLocalizedString("New tag, hold to rename", comment: "Create a new tag")
        save()
        return true
    }
    ...
    func newIssue() {
        ...
        // it wont appear in the list of issues they see.
